...
and folds into a complex three dimensional structure 
which usually determines the protein's function and 
...
remarkable insights.
A cell in the lining of your stomach would have a 
...
this type of mass analyser is termed \acrfull{tof} as 
...
to acheive comparable results \citep{Groseclose2008}.
...
\begin{figure}[ht]
 \begin{center}
  ...
  highlight tumour regions, see \cite{Mittal2016}.
...
\gls{maldi}-\gls{ims}:
\begin{itemize}
  \item Ovarian Cancer --- Peptide data from full 
  sections of ovarian tumours embedded in surrounding
  peritoneal tissue.
  
  \item Murine Glycans --- N-linked glycan data from 
  sections of murine kidney, part treated with PNGase F,
  part untreated.
  
  \item \glspl{tma} --- Peptide data from \glspl{tma}. 
  We consider two sub-applications. Endometrial cancer
  forms our main interest, and vulvar cancer forms a 
  secondary interest which we mainly use for 
  replicating results.
...
The ovarian cancer data we will consider relate to 
tumours embedded in peritoneal tissue (elements of the 
internal abdominal wall) from three ovarian cancer 
patients.
Further details can be found in the work of 
\citet{Gustafsson2012thesis}, and for some motivation 
on why the study of ovarian cancer is of importance, 
we quote \citet{Winderbaum2015}:
\begin{quote}
...
proteins in \reftab{massIDs} on the ovarian cancer
patients A and B as introduced in 
\reftab{imgBasicStats}.
...
of these data and, amongst other things, that these 
data contain spectra from both tumour tissue and 
surrounding non-tumour tissues excised as a 
...
are present more in tumour while being less prevalent 
in the surrounding non-tumour tissues.
We approach this in a two-step process, first using
$k$-means clustering to separate the tissue types in 
an automated manner, second using a \gls{dipps}-based
...
that may be over-expressed by tumour cells in 
particular patients but not others. 
...
These glycan data relate to sections 
...
\gls{maldi}-\gls{ims}.
Further details are provided by \citet{Gustafsson2015},
who also provide some motivation for why the study of 
glycans is of interest:
\begin{quote}
...
number of patients ($43$), as discussed in 
\refsec{TMAs}. 
The diagnostic variable of interest for prediction is 
\gls{lnm} status, which we attempt to 
classify on the basis of the \gls{maldi}-\gls{ims}
data in \refchap{classificationApplication}.
Further details on the acquisition of these data can 
be found in the work of \citet{Mittal2016}, and 
\citet{Winderbaum2016} provide some motivatation on 
why the the classification of \gls{lnm} in 
endometrial cancer is of interest:
\begin{quote}
...
to treatment \citep{Jacques1998}. 
Although the presence of \gls{lnm} is confirmed in only 
around $15\%$ of cases \citep{Morrow1991,Creasman2006}, 
...
tissue was manually annotated by a pathologist --- see
\cite{Mittal2016}.
...
print(xtable(temp, 
      ...
      add.to.row = list(pos=list(-1,
                                 nrow(temp)),
                        command = c(paste("\\toprule \n",
                                          "\\multirow{2}{*}{Patient \\#} & \\multirow{2}{*}{\\gls{lnm}} & \\multirow{2}{*}{Grade} & \\multirow{2}{*}{Size} & Myometrium & \\multirow{2}{*}{\\# Spectra} & \\# Cancer & \\# Empty \\\\ \n & & & & Invasion & & Spectra & Spectra \\\\ \n",
                                          "\\midrule \n"),
                                    "\\bottomrule \n")
      )
)
...
The vulvar cancer data relate to another 
gynaecological cancer for which, similar to 
endometrial cancer, \gls{lnm} is highly predictive of 
prognosis and important to the choice of treatment.
While providing access to a smaller cohort (28) than
the endometrial cancer data, the vulvar cancer data 
represents an independant dataset on a different 
(although related) type of cancer.
...
below, which can is more computationally efficient 
compared to hierarchical methods that compute full 
dendrograms.
...
parallel, and compared to hierachical methods that 
generate full dendrograms and are difficult to 
parallelise it can be more computationally efficient.
...
and included in the more broad term supervised learning, 
