...
results as many standard methods are 
inappropriate.
...
\cite{Deininger2008,Alexandrov2010} and 
...
generally on any binary presence/ absence data as 
introduced in \refdef{presAbsData}.
...
\refsec{binarysummedbinaryequivalence}:
\begin{itemize}
...
In actual fact, spectra were collected from 
...
<<figure_A3_binary_clusterings, dependson=c("clus_A3_binary","A3_load"), fig.show='hold', results='hide', out.width="0.9\\linewidth", fig.width=6, fig.height=4, fig.cap= "The results of $4$-means clustering (\\refalg{kmeans}) on the dataset A3. Analysis was done using the cosine (left), Euclidean (centre) and Hamming (right) distances on the binned (bin size $0.25$) binary data. Apart from minor speckling effects, spatially localised clusters that well separate the main tissue types are apparent in all three clustering results.", fig.align='center'>>=
...
<<figure_A3_non_binary_euc_clusterings, dependson=c("clus_A3_non_binary_euc","A3_load"), fig.show='hold', results='hide', out.width="0.9\\linewidth", fig.width=6, fig.height=4, fig.cap= "The results of $4$-means clustering (\\refalg{kmeans}) on the dataset A3. Analysis was done using the Euclidean distance on binned (bin size $0.25$); intensity (left), area (centre) and \\gls{snr} (right) data. All three clustering results show spatially de-localised clusters. Although the clustering results show various degrees of success at separating certain features of the tissue, all fail to separate the main tissue types present in the tissue section.", fig.align='center'>>=
mI.m <- spatialPlot(clus_A3,fExists,
...
<<figure_A3_non_binary_cos_clusterings, dependson=c("clus_A3_non_binary_cos","A3_load"), fig.show='hold', results='hide', out.width="0.9\\linewidth", fig.width=6, fig.height=4, fig.cap= "The results of $4$-means clustering (\\refalg{kmeans}) on the dataset A3. Analysis was done using the cosine distance on binned (bin size $0.25$); intensity (left), area (centre) and \\gls{snr} (right) data.", fig.align='center'>>=
mI.m <- spatialPlot(clus_A3,fExists,
...
\draw (5.8,-0.15) node {
<<figure_A3_binary_cos_clustering, dependson=c("clus_A3_smoothed_cos","A3_load"), message=FALSE, fig.width=3.6, out.width="0.518\\linewidth">>=
mI.m <- spatialPlot(clus_A3,fExists,
                    plot_var = 'Binary.Cosine',
                    plot_var_type = 'categorical',
                    minX_in = minX,
...
follow-up studies, we prioritise our efforts on 
...
\begin{itemize}
  ...
  simply I will continue consider only one binning for 
