...
over non-linear alternatives in \gls{hdlss} contexts.
...
$\bm{d}_{\LDA}$ can be thought of as a direction or 
line which `best separates' the two classes, as 
...
can be thought of as different approaches to 
...
a compromise somewhere inbetween using separate 
...
\begin{equation}
  (\diag{\hat{W}})^{-1}\hat{B}.
	\label{eqn:dNB}
\end{equation}
In the case that the variables are infact independent, 
...
This estimation should infact be conservative, as in 
...
this poses a challenge for classification.
